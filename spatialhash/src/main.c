...
void main_loop() {
  ...
  player_hitbox.aabb.max.x = player_pos.x + player->material->albedo_texture->width;
  player_hitbox.aabb.max.y = player_pos.y + player->material->albedo_texture->height;

  player_hitbox_c.center.x = player_pos.x + (float)player->material->albedo_texture->width / 2.0f;
  player_hitbox_c.center.y = player_pos.y + (float)player->material->albedo_texture->height / 2.0f;
...
int main(int argc, char *argv[])
{
  ...
  player_hitbox_c = binocle_collider_circle_new(texture->width > texture->height ? (texture->width / 2.0f) : (texture->height / 2.0f), player_center);
  ...
  enemy_material->albedo_texture = enemy_texture;
  ...
  enemy_hitbox_c = binocle_collider_circle_new(enemy_texture->width > enemy_texture->height ? (enemy_texture->width / 2.0f) : (enemy_texture->height / 2.0f), enemy_center);
  ...
  binocle_sprite_destroy(enemy);
  binocle_material_destroy(enemy_material);
  binocle_texture_destroy(enemy_texture);
  binocle_image_destroy(enemy_image);
  binocle_sprite_destroy(player);
